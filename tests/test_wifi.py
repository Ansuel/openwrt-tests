...
def test_wifi_hwsim_sae_mixed(ssh_command):
    ...
    assert "auth" in "\n".join(
        ssh_command.run("ubus -t 20 subscribe hostapd.phy0-ap0 | grep '\"auth\":' | while read line; do echo auth && killall ubus; done")[0]
    )
    ...
    assert "disassoc" in "\n".join(
        ssh_command.run("ubus -t 20 subscribe hostapd.phy0-ap0 | grep '\"disassoc\":' | while read line; do echo disassoc && killall ubus; done")[0]
    )
